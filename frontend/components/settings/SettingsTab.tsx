...
export default function SettingsTab() {
  ...
  return (
    <div className="flex flex-col h-full bg-bg-primary">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-border min-h-[52px]">
        <button onClick={() => setActiveTab('chats')} className="p-1 rounded-full hover:bg-bg-hover text-label-secondary">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <h1 className="text-lg font-bold text-label-primary">Settings</h1>
      </div>

      <div className="flex-1 overflow-y-auto">
        {/* Profile header */}
        <div className="px-4 py-4 flex items-center gap-4 border-b border-border">
          <div className="w-14 h-14 rounded-full bg-brand/20 flex items-center justify-center text-brand text-lg font-bold">
        ...
        {sections.map((section) => (
          <div key={section.title} className="border-b border-border">
            <p className="px-4 py-2 text-xs font-semibold text-brand uppercase tracking-wide">{section.title}</p>
